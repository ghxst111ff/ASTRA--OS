/* =========================
   ...
   v4.6 — top-down interception no longer swallows normal messages
========================= */
...
ASTRA.modules.command = {
    ...
    process(command){
        const rawCommand=String(command??"").trim();if(!rawCommand)return false;
        const topDown=ASTRA.modules.topDownCoach;
        if(topDown?.handle){try{
            const result=topDown.handle(rawCommand);
            if(result&&typeof result.then==="function"){
                result.then(resolved=>{
                    if(resolved?.handled||resolved?.started){this.speakTopDownResult(resolved);return;}
                    this.processNormal(rawCommand);
                }).catch(error=>{console.error("ASTRA top-down routing:",error);this.processNormal(rawCommand);});
                return true;
            }
            if(result?.handled||result?.started){this.speakTopDownResult(result);return true;}
        }catch(error){console.error("ASTRA top-down routing:",error);}
        return this.processNormal(rawCommand);
    }
};
ASTRA.registerModule("command",ASTRA.modules.command);console.log("ASTRA Command Router v4.6 Loaded — normal messages pass through after unhandled top-down checks");