...
const TopDownCoach=(()=>{
 ...
 function buildReviewPrompt(tf,analysis,system){const req=requirementsFor(tf);return `You are VEGA, acting as Jay's top-down trading mentor. Review ONLY the current ${tf} timeframe analysis against Jay's original Trading Strategy and the timeframe-specific contract below. TIMEFRAME ROLE: ${roleFor(tf)}. REQUIRED ${tf} CONTRACT: ${req.map((x,i)=>`${i+1}) ${x}`).join("; ")}. JAY'S SPOKEN ANALYSIS: ${analysis||"(No explanation captured.)"}. Inspect the shared chart and compare what Jay said with what is visible, but do not invent details you cannot see.\n\nSTRICT REVIEW MODEL:\n1. A requirement is COVERED when Jay explicitly addresses it, including a negative/absence assessment such as 'I don't see EQH/EQL/trendline liquidity.' Do not demand a stronger version of an already-covered requirement.\n2. A requirement is MISSING only when Jay did not address that requirement at all.\n3. A recommendation, preferred method, extra detail, or your own trading opinion is NOT a missing requirement.\n4. Never import requirements from another timeframe.\n5. For liquidity, do not replace Jay's assessment with your preferred liquidity targets or clusters. Only call it incorrect if the visible chart clearly contradicts what Jay said.\n6. For invalidation, a clear conditional invalidation satisfies the requirement unless the current contract explicitly asks for a numeric level.\n7. Return missing items ONLY as exact requirement names from the contract.\nReturn JSON only: {"status":"CORRECT"|"CORRECTION"|"UNCLEAR","coveredRequirements":[exact contract names],"missingRequirements":[exact contract names],"correctPoints":[...],"incorrectPoints":[...],"feedback":"short spoken feedback","reason":"brief reason"}. Trading strategy context: ${JSON.stringify(system)}`;}
 ...
 function transcriptCoverage(tf,analysis){const a=String(analysis||"").toLowerCase();if(tf!=="WEEKLY")return {};return {
  ...
  "if-this-then-that scenarios":/\bif\b/.test(a)&&(/\b(price|break|tap)\b/.test(a)),
