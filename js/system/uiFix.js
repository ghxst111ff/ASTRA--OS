...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function render(module, tab) {
        ...
        if (module === "journal") {
            ...
            target.innerHTML = {
                ...
                notes: `<div class="content-card"><h3>NOTES</h3>${trades.filter(trade => trade.notes).map(trade => `
                    <div class="trade-line"><b>${esc(trade.pair || "NOTE")}</b><span>${esc(trade.direction || "")}</span><small>${esc(trade.notes)}</small></div>
                `).join("") || '<p class="empty-state">No journal notes yet.</p>'}</div>`
            }[tab] || "";
        }

        if (module === "trading") {
            target.innerHTML = {
                watchlist: `<div class="content-card"><h3>WATCHLIST</h3>
                    <div class="watch-row"><b>XAUUSD</b><span>Live</span><strong>+0.35%</strong><em>↑</em></div>
                    <div class="watch-row"><b>GBPUSD</b><span>Live</span><strong class="negative">-0.12%</strong><em class="negative">↓</em></div>
                    <div class="watch-row"><b>EURUSD</b><span>Live</span><strong>+0.18%</strong><em>↑</em></div>
                    <div class="watch-row"><b>NAS100</b><span>Live</span><strong>+0.41%</strong><em>↑</em></div>
                </div>`,
                ...
                sessions: `<div class="content-card"><h3>SESSIONS</h3>
                    <div class="metric"><span>London</span><b>7:00 AM – 11:00 AM</b></div>
                    <div class="metric"><span>New York</span><b>1:00 PM – 4:00 PM</b></div>
                    <div class="metric"><span>Review</span><b>8:30 PM – 9:00 PM</b></div>
                </div>`
            }[tab] || "";
        }
        ...
        if (module === "backtest") {
            ...
            target.innerHTML = {
                overview: `<div class="stats-grid">
                    <div><small>Total Trades</small><b>${insight.total ?? trades.length}</b></div>
                    <div><small>Win Rate</small><b>${insight.winRate ?? 0}%</b></div>
                    <div><small>Wins</small><b>${insight.wins ?? 0}</b></div>
                    <div><small>Net P/L</small><b class="positive">${insight.netPnl ?? 0}</b></div>
                </div><div class="content-card"><h3>BACKTEST COACH</h3><p class="empty-state">Backtesting stays completely separate from live trading.</p></div>`,
    ...
    console.log("ASTRA UI Fix v2.0 Loaded — views only");
});
