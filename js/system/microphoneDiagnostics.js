/* VEGA MICROPHONE DIAGNOSTICS v1.1
   Standalone microphone input diagnostic.
   Does not modify dashboard buttons, layout, or SpeechRecognition. */
const MicrophoneDiagnostics = (() => {
    ...
    const test = async (durationMs = 3500) => {
        stop();
        if (!navigator.mediaDevices?.getUserMedia) return {ok:false,code:"unsupported",message:"Microphone diagnostics are not supported by this browser."};
        try {
            activeStream = await navigator.mediaDevices.getUserMedia({audio:true});
            const AudioCtx = window.AudioContext || window.webkitAudioContext;
            if (!AudioCtx) { stop(); return {ok:false,code:"audio-context-unsupported",message:"Microphone access works, but input-level measurement is not supported here."}; }
            ...
            while (performance.now()-started < durationMs) {
                ...
                sumRms += Math.sqrt(sum/data.length); peak=Math.max(peak,localPeak); samples++;
            ...
            const message=level==="strong"?"Microphone input is strong. Try normal-speed speech in VEGA voice.":level==="usable"?"Microphone input is usable. If normal speech still transcribes poorly, the recognition service is likely the limiting layer.":"Microphone input is quiet. Check the selected microphone, permission, distance, and input level before testing recognition again.";
            ...
            const name=e?.name||"UnknownError"; stop();
    ...
    return {name:"Microphone Diagnostics",version:"1.1",test,stop,status};
})();
