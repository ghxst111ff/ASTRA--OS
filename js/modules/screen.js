...
const ScreenModule=(()=>{
    ...
    function stopCapture(reply=true){ASTRA.modules.coach?.stopObserver?.();if(stream){stream.getTracks().forEach(track=>track.stop());stream=null;}if(video){video.srcObject=null;video.remove();video=null;}if(reply)AstraReply("Screen sharing stopped.");}
    ...
    return{name:"Screen View",version:"2.3",open,close,startCapture,stopCapture,getFrame,status,analyze,getAnalysis,get sharing(){return!!stream;},commands:[{trigger:"open screen",action:open},{trigger:"close screen",action:close},{trigger:"screen status",action:status},{trigger:"share screen",action:startCapture},{trigger:"stop screen",action:stopCapture},{trigger:"screen analysis",action:showAnalysis}]};
})();
ASTRA.registerModule("screen",ScreenModule);ScreenModule.commands.forEach(command=>ASTRA.commands.push(command));console.log("ASTRA Screen View v2.3 Loaded");
