/* =========================================
   ASTRA SCREEN VIEW MODULE v2.4
   Screen capture + chart frame access + quiet top-down mode
========================================= */
const ScreenModule=(()=>{
    ...
    function open(){const screen=panel();if(!screen){if(!ASTRA.modules.topDownCoach?.snapshot?.().active)AstraReply("Screen panel not found.");return false;}screen.style.display="block";screen.classList.add("active");return true;}
    function close(){stopCapture(false);const screen=panel();if(screen){screen.style.display="none";screen.classList.remove("active");}if(!ASTRA.modules.topDownCoach?.snapshot?.().active)AstraReply("Screen view closed.");}
    async function startCapture(){
        if(!navigator.mediaDevices?.getDisplayMedia){if(!ASTRA.modules.topDownCoach?.snapshot?.().active)AstraReply("Screen capture is not supported by this browser.");return false;}
        const topDownActive=!!ASTRA.modules.topDownCoach?.snapshot?.().active;
        open();
        try{
            ...
            // Top-down analysis is a deliberate quiet listening state. Sharing the screen
            // must never trigger a greeting, market commentary, or proactive observation.
            if(!topDownActive) {
                AstraReply("Screen sharing connected.");
                if(!ASTRA.modules.topDownCoach?.snapshot?.().active)ASTRA.modules.coach?.startObserver?.(12000);
            } else {
                ASTRA.modules.coach?.stopObserver?.();
            }
            return true;
        }catch(error){console.error("ASTRA screen capture:",error);if(!topDownActive)AstraReply("Screen sharing was cancelled or blocked.");return false;}
    }
    function stopCapture(reply=true){ASTRA.modules.coach?.stopObserver?.();if(stream){stream.getTracks().forEach(track=>track.stop());stream=null;}if(video){video.srcObject=null;video.remove();video=null;}if(reply&&!ASTRA.modules.topDownCoach?.snapshot?.().active)AstraReply("Screen sharing stopped.");}
    ...
    return{name:"Screen View",version:"2.4",open,close,startCapture,stopCapture,getFrame,status,analyze,getAnalysis,get sharing(){return!!stream;},commands:[{trigger:"open screen",action:open},{trigger:"close screen",action:close},{trigger:"screen status",action:status},{trigger:"share screen",action:startCapture},{trigger:"stop screen",action:stopCapture},{trigger:"screen analysis",action:showAnalysis}]};
})();
ASTRA.registerModule("screen",ScreenModule);ScreenModule.commands.forEach(command=>ASTRA.commands.push(command));console.log("ASTRA Screen View v2.4 Loaded");
